    ...
    enum device_endian endian)
{
    ...
    if (l < 4 || !IS_DIRECT(mr, false)) {
        ...
        if (panda_use_memcb)
            panda_callbacks_before_dma(NULL, addr1, ptr, 4, false);
        switch (endian) {
        ...
        if (panda_use_memcb)
            panda_callbacks_after_dma(NULL, addr1, ptr, 4, false);
        r = MEMTX_OK;
    }
    if (result) {
        *result = r;
    ...
    enum device_endian endian)
{
    ...
    if (l < 8 || !IS_DIRECT(mr, false)) {
        ...
        ptr = MAP_RAM(mr, addr1);
        if (panda_use_memcb)
            panda_callbacks_before_dma(NULL, addr1, ptr, 8, false);
        switch (endian) {
        case DEVICE_LITTLE_ENDIAN:
            val = ldq_le_p(ptr);
            break;
        ...
        if (panda_use_memcb)
            panda_callbacks_after_dma(NULL, addr1, ptr, 8, false);
        r = MEMTX_OK;
    }
    if (result) {
        *result = r;
    ...
    hwaddr addr, MemTxAttrs attrs, MemTxResult *result)
{
    ...
    if (!IS_DIRECT(mr, false)) {
        ...
        ptr = MAP_RAM(mr, addr1);
        if (panda_use_memcb)
            panda_callbacks_before_dma(NULL, addr1, ptr, 1, false);
        val = ldub_p(ptr);
        if (panda_use_memcb)
            panda_callbacks_after_dma(NULL, addr1, ptr, 1, false);
        r = MEMTX_OK;
    }
    if (result) {
        *result = r;
    ...
    enum device_endian endian)
{
    ...
    if (l < 2 || !IS_DIRECT(mr, false)) {
        ...
        ptr = MAP_RAM(mr, addr1);
        if (panda_use_memcb)
            panda_callbacks_before_dma(NULL, addr1, ptr, 2, false);
        switch (endian) {
        case DEVICE_LITTLE_ENDIAN:
            val = lduw_le_p(ptr);
            break;
        ...
        if (panda_use_memcb)
            panda_callbacks_after_dma(NULL, addr1, ptr, 2, false);
        r = MEMTX_OK;
    }
    if (result) {
        *result = r;
    ...
    hwaddr addr, uint32_t val, MemTxAttrs attrs, MemTxResult *result)
{
    ...
    if (l < 4 || !IS_DIRECT(mr, true)) {
        ...
        ptr = MAP_RAM(mr, addr1);
        if (panda_use_memcb)
            panda_callbacks_before_dma(NULL, addr1, ptr, 4, true);
        stl_p(ptr, val);
        if (panda_use_memcb)
            panda_callbacks_after_dma(NULL, addr1, ptr, 4, true);
    ...
    hwaddr addr, uint32_t val, MemTxAttrs attrs, MemTxResult *result)
{
    ...
    if (!IS_DIRECT(mr, true)) {
        ...
        ptr = MAP_RAM(mr, addr1);
        if (panda_use_memcb)
            panda_callbacks_before_dma(NULL, addr1, ptr, 1, true);
        stb_p(ptr, val);
        if (panda_use_memcb)
            panda_callbacks_after_dma(NULL, addr1, ptr, 1, true);
    ...
    MemTxResult *result, enum device_endian endian)
{
    ...
    if (l < 2 || !IS_DIRECT(mr, true)) {
        ...
        ptr = MAP_RAM(mr, addr1);
        if (panda_use_memcb)
            panda_callbacks_before_dma(NULL, addr1, ptr, 2, true);
        switch (endian) {
        case DEVICE_LITTLE_ENDIAN:
            stw_le_p(ptr, val);
            break;
        ...
        if (panda_use_memcb)
            panda_callbacks_after_dma(NULL, addr1, ptr, 2, true);
    ...
    MemTxResult *result, enum device_endian endian)
{
    ...
    if (l < 8 || !IS_DIRECT(mr, true)) {
        ...
        ptr = MAP_RAM(mr, addr1);
        if (panda_use_memcb)
            panda_callbacks_before_dma(NULL, addr1, ptr, 8, true);
        switch (endian) {
        case DEVICE_LITTLE_ENDIAN:
            stq_le_p(ptr, val);
            break;
        ...
        if (panda_use_memcb)
            panda_callbacks_after_dma(NULL, addr1, ptr, 8, true);
